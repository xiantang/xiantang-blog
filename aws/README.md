...
只读，只跑 `describe`/`list`，不删任何东西。查六类：
...
| S3 未完成分段上传 | 按存储 | **控制台的对象列表里看不到**，见下 |

脚本里的单价是量级估算，只为让数字有体感，**准确数字看 Cost Explorer**。

⚠️ 前五类的 API 都是 **region-scoped**。手滑最容易发生在你不常看的 region
（跟着某个教程在 `us-east-1` 建了东西然后忘了），所以偶尔要跑一次 `--all`。

⚠️ 跟 `ce` 不同，`ec2 describe-*` 这些**不收费**，放心跑。

**判断标准**：这个账号目前的架构里，前五类应该**全是空的**。
唯一在计费的公网 IPv4 是挂在 EC2 实例上正在用的那个（~$1.9/月），
它不是孤儿，脚本也不会报它。任何一条有输出，都值得停下来搞清楚是谁建的。

### S3 为什么单独一节

S3 **不进 region 循环** —— 桶列表是全局的，塞进去的话 `--all` 会把同一个桶报 30 遍。
所以加不加 `--all`，S3 都只查一遍。

查的东西和别的类不一样，因为 **空桶不花钱**，按「不用也在计费」的定义它根本不算孤儿。
真正该抓的是**未完成的分段上传**：上传大文件中断后，已传的分片留在桶里，
**控制台的对象列表完全看不到它们**，但照常按存储计费。日积月累能到几十 GB，
而且因为看不见，通常是从账单反推才发现的。

治本不是手动 abort，是给桶加一条 lifecycle 规则让分片自动过期：
...
脚本还会列出超过 1GB 的桶及其大小（走 CloudWatch 的 `BucketSizeBytes`，
比 `list-objects` 累加快几个数量级，且在免费额度内），
并标出哪些桶**没有 lifecycle 规则** —— 那意味着它只进不出。

这不是为了省那几毛钱，是 2026-08-03 磁盘写满那次的同一个教训：
**一个缓慢增长又没人看着的资源，迟早会以别的形式还回来。**

⚠️ 存 Terraform state 的那个桶（`xiantang-tfstate-*`）**故意不归 Terraform 管**，
所以它会出现在这里但在任何 `.tf` 里都找不到。这是「鸡生蛋」问题的取舍，不是遗漏。
